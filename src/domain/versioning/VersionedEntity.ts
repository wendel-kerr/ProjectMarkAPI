...
/**
 * Simple append-only version manager to be COMPOSED inside aggregates.
 * Provides immutable history semantics.
 */
export abstract class VersionedEntity<TData> implements IVersioned<TData> {
  ...
  /**
   * Shallow-merge update into previous data to create a new immutable snapshot.
   */
  ...
  public getLatestVersion(): IVersion<TData> {
    if (this.versions.length === 0) {
      throw new VersionNotFoundError(1);
    }
