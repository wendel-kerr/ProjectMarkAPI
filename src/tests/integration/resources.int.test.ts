...
describe('Resources CRUD (Phase 6)', () => {
  it('creates, reads, lists by topic, updates and deletes resources; tree includes resources', async () => {
    ...
    const r1get = await request(app)
      .get(`/resources/${r1.body.id}`)
      .set('Authorization', `Bearer ${editorToken}`);
    ...
    const del = await request(app)
      .delete(`/resources/${r2.body.id}`)
      .set('Authorization', `Bearer ${editorToken}`);
    expect(del.status).toBe(204);

    const listChild = await request(app)
      .get('/resources')
      .set('Authorization', `Bearer ${editorToken}`)
      .query({ topicId: childId });
